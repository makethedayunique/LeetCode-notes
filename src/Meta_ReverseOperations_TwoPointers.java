 ...
 * Reverse Operations
 * 
 * You are given a singly-linked list that contains N integers. A subpart of the list is a contiguous set of 
 * even elements, bordered either by either end of the list or an odd element. For example, 
...
class Main {
  ...
  Node reverse(Node head) {
    ...
    newhead.next = head;
    Node slowprev = newhead;
    Node fastprev = newhead;
    Node slow = head;
    Node fast = head;
    while (fast != null) {
      while (slow!= null && (slow.data & 1) != 0) {
        slowprev = slow;
        fastprev = fast;
        slow = slow.next;
        fast = fast.next;
      }
      while (fast != null && (fast.data & 1) == 0) {
        fastprev = fast;
        fast = fast.next;
      }
      // Start reverse
